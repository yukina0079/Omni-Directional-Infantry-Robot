...
#define GYRO_CALIB_SAMPLES    800u
#define GYRO_CALIB_DISCARD    150u
#define GYRO_CALIB_MAX_TRY    1600u
#define ACCEL_STILL_MIN       0.85f
#define ACCEL_STILL_MAX       1.15f
#define GYRO_STILL_DPS        8.0f
#define IMU_FAIL_FREEZE_N     20u
...
static uint8_t imu_gyro_calibrate(void)
{
    ...
    /* Gyro digital filter needs time after reset; first frames are not bias. */
    for (tries = 0; tries < GYRO_CALIB_DISCARD; tries++)
    {
        (void)BMI055_update(&accel[0], &accel[1], &accel[2],
    ...
    while ((good < GYRO_CALIB_SAMPLES) && (tries < GYRO_CALIB_MAX_TRY))
    {
        ...
        tries++;
        st = BMI055_update(&accel[0], &accel[1], &accel[2],
                           &gyro[0], &gyro[1], &gyro[2], &temp[0]);
        if (st != BMI055_OK)
        {
            delay_ms(1);
    ...
    {
        printf("IMU calib aborted: still %u/%u acc=%.3f g gyro=%.2f (%.1f %.1f %.1f)\r\n",
...
static void imu_set_healthy(uint8_t healthy)
{
    imu_healthy = healthy;
    /* ALIENTEK-style LED is typically active-low: 0 = on */
    if (healthy)
    {
        LED0(1);
    }
...
void imu_updata(void)
{
    ...
    INS_angle_filtered[0] = low_pass_filter(INS_continuous_angle[0], &angle_lpf[0], imu_dt);
    INS_angle_filtered[1] = low_pass_filter(INS_continuous_angle[1], &angle_lpf[1], imu_dt);
    INS_angle_filtered[2] = low_pass_filter(INS_continuous_angle[2], &angle_lpf[2], imu_dt);
}
