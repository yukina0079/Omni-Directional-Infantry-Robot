...
void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi)
{
		if(hspi->Instance == SPI1){
				...
				__HAL_RCC_GPIOA_CLK_ENABLE();
				gpio_initstruct.Pin = GPIO_PIN_1;//使能
				gpio_initstruct.Mode = GPIO_MODE_OUTPUT_PP;//推挽输出
				gpio_initstruct.Pull = GPIO_PULLUP;//配置上拉下拉
				gpio_initstruct.Speed = GPIO_SPEED_FREQ_HIGH;//配置输出速度	
				HAL_GPIO_Init(GPIOA,&gpio_initstruct);
				
				gpio_initstruct.Pin = GPIO_PIN_0;//中断
				gpio_initstruct.Mode = GPIO_MODE_INPUT;//输入
				HAL_GPIO_Init(GPIOA,&gpio_initstruct);
		}
...
{
    NRF24L01_CE_LOW();  // 拉低CE，进入配置模式（停止当前模式）
    ...
    NRF24L01_CE_HIGH();  // 拉高CE，进入接收状态
    HAL_Delay(100);   // 延时保证配置生效，避免模式冲突
}
...
void B_nrf24l01_switch_tx_to_C(void)
{
    ...
    HAL_Delay(100);   // 延时保证配置生效，无需立即拉高CE（发送时由nrf24l01_tx_packet触发）
}

/**
 * @brief  C模块配置：接收B转发的数据（使用B-C通信参数）
 * @note   基于库函数nrf24l01_rx_mode()修改，替换地址和频道为ADDR_BC、CH_BC
 */
void C_nrf24l01_rx_from_B(void)
{
    NRF24L01_CE_LOW();  // 拉低CE，进入配置模式
    
    // 1. 写RX_ADDR_P0（B→C的地址 ADDR_BC）
    NRF24L01_Write_Buf(NRF_WRITE_REG + RX_ADDR_P0, ADDR_BC, ADDR_BC_LEN);
    
    // 2. 库函数默认配置（自动应答、数据宽度等，保持不变）
    NRF24L01_Write_Reg(NRF_WRITE_REG + EN_AA, 0x01);        // 使能通道0自动应答（向B回传ACK）
    NRF24L01_Write_Reg(NRF_WRITE_REG + EN_RXADDR, 0x01);    // 使能通道0接收地址
    NRF24L01_Write_Reg(NRF_WRITE_REG + RF_CH, CH_BC);       // 配置为B-C频道 40
    NRF24L01_Write_Reg(NRF_WRITE_REG + RX_PW_P0, DATA_WIDTH); // 数据长度32字节
    NRF24L01_Write_Reg(NRF_WRITE_REG + RF_SETUP, 0x0f);     // 0db增益，2Mbps，低噪声增益开启
    NRF24L01_Write_Reg(NRF_WRITE_REG + CONFIG, 0x0f);       // 接收模式，PWR_UP，EN_CRC
    
    NRF24L01_CE_HIGH();  // 拉高CE，进入持续接收状态
