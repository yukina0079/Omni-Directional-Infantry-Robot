...
int main(void)
{
     ...
     */
    /*
     * 0.25 V/deg + 1.4 V Coulomb followed the stick but shook the
     * turret: P kicked each encoder step and Coulomb slammed when
     * the following error crossed ~0.8 deg. Drop P a notch, double
     * D, soften the voltage slew. Lead 8.6 deg still gives
     * 0.18*8.6 + 0.8 ≈ 2.3 V -- enough to keep moving, less buzz.
     */
    PID_Pos_Set(0.18f, 0.0f, 0.0012f, 5.5f);
    PID_Vel_Set(0.01f, 0.0f, 0.0f, 2.5f);
    PID_Cur_Set(0.5f, 0.0f, 0.0f, 2.5f);

    PID_Pos.D_lpf_Tf = 0.004f;

    /* 2.3 -> 5.5 V in ~40 ms once the rail blend turns on. */
    PID_Pos.output_ramp = 80.0f;
    PID_Vel.output_ramp = 50.0f;    /* volts per second (gentler than 100) */
    PID_Cur.output_ramp = 50.0f;    /* volts per second */
...
#else
    Motor_init();       /* sweeps the rotor, then stops the motor */
#endif
