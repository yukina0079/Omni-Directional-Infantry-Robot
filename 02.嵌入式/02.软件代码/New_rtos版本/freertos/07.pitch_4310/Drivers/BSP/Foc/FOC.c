...
{
	float Uout;
	float vector_voltage;
	uint32_t sector;
	float T0,T1,T2;
	float Ta,Tb,Tc;

	/* The position controller is voltage-mode, not current-mode. Limit the
	 * commanded voltage vector here so every caller observes the same torque
	 * ceiling, including alignment and diagnostic paths. */
	vector_voltage = sqrtf(Uq * Uq + Ud * Ud);
	if (vector_voltage > PITCH_TORQUE_LIMIT_V) {
		float scale = PITCH_TORQUE_LIMIT_V / vector_voltage;
		Uq *= scale;
		Ud *= scale;
	}
